...
create table if not exists `VuelosApp`.`users` (
    ...
    `contraseña` VARCHAR(255) NOT NOT NULL,
...
create table if not exists `VuelosApp`.`flights` (
    ...
    `montoVuelo` INT UNSIGNED NULL,
    `origen` VARCHAR(255) NULL,
    `destino_id` INT UNSIGNED NOT NULL,
    PRIMARY KEY (`id`),
    FOREIGN KEY (`destino_id`) REFERENCES `VuelosApp`.`destinies`(`id`) ON DELETE CASCADE
);

insert into VuelosApp.flights values(1,'2026-02-08 10:00','2026-02-08 18:00',6,'American Airlines',200, 800, 'Buenos Aires',1);
...
create table if not exists `VuelosApp`.`trips` (
    ...
    `valor_reserva` INT UNSIGNED NOT NULL,
    ...
    FOREIGN KEY (`flight_id`) REFERENCES `VuelosApp`.`flights`(`id`));

insert into VuelosApp.trips values(1,'2026-10-01',1000,'pendiente',1,1,1);